...
## 핵심 기능
- **Elasticsearch 기반 통합 검색 시스템:**
  - **스마트 레시피 검색:** 검색 가중치(Boosting)를 적용하여 정확도를 최적화했습니다. (레시피명: **5.0**, 재료: **1.0**)
  - **카테고리별 질병 검색:** Elasticsearch의 `NativeQuery`를 활용하여 카테고리 내 질병명을 고속 검색하며, 검색 결과가 있는 카테고리만 동적으로 노출합니다.
- **질환 기반 레시피 필터링:** - 특정 질환 선택 시, 해당 질환의 **주의 식품**을 포함하는 레시피를 자동으로 제외하여 안전한 식단을 제안합니다.
- **레시피 검색 경고:** - 레시피 검색 시 주의 식품 포함 여부를 체크하여, 포함된 경우 사용자에게 **경고 문구**를 함께 제공합니다.
...
- **Search Engine:** **Elasticsearch** (Spring Data Elasticsearch / NativeQuery 활용)
- **Database:**
...
## Roadmap & Updates
- [X] ElasticSearch 도입 및 검색 성능 최적화
